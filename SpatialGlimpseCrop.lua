...
------------------------------------------------------------------------
local SpatialGlimpse, parent = torch.class("nn.SpatialGlimpseCrop", "nn.Module")

function SpatialGlimpse:__init(size, depth, scale, resize, imageSize)
   require 'nnx'
   require 'cuimage'
   ...
   self.imageSize = imageSize or 224
...
function SpatialGlimpse:updateOutput(inputTable)
   ...
   local input, location, step = unpack(inputTable)
   ...
   if not self.resize then
       ...
       self.output:resize(input:size(1)*self.depth*input:size(2), self.height, self.width)
       self.output = cuimage.scale(self.output, self.imageSize, self.imageSize)
       self.output:resize(input:size(1), self.depth*input:size(2), self.imageSize, self.imageSize)
   end
...
function SpatialGlimpse:updateGradInput(inputTable, gradOutput)
   ...
   if not self.resize then
       self.gradInput = {input.new():resizeAs(input):zero(), location.new():resizeAs(location):zero()}
   else
