...
require 'cutorch'
require 'cunn'
require 'VRClassReward'
require '../utils/Recurrent'
require '../utils/Recursor'
-- References :
...
cmd:option('--testOnly', false, 'test model')
...
cmd:option('--e', 0.2, 'param for e-greedy')
cmd:option('--greedy', false, 'use e-greedy')
cmd:option('--discount', 0.8, 'discount of e-greedy')
...
print('model...')
--[[Model]]--
if opt.xpPath ~= '' and not opt.testOnly then
   ...
   loadModel = torch.load(opt.xpPath)
   rnn = loadModel:model():get(1):get(2).rnn
   locator = loadModel:model():get(1):get(2).action
   decider = loadModel:model():get(1):get(2).action2
   classify = loadModel:model():get(1):get(4)
  
   rnn.accGradParameters = function(self) end
   locator.accGradParameters = function(self) end
   rnn.parameters = function() return nil end
   locator.parameters = function() return nil end
   -- actions (decider)
   
   Decider = nn.Sequential()
   Decider:add(nn.Linear(opt.hiddenSize, opt.hiddenSize))
   Decider:add(nn.ReLU())
   Decider:add(nn.Linear(opt.hiddenSize, 1))
   Decider:add(nn.HardTanh(-10,10))
   Decider:add(nn.Sigmoid()) -- bounds mean between -1 and 1
   Decider:add(nn.ReinforceBernoulli(opt.stochastic)) -- sample from normal, uses REINFORCE learning rule

   local method = 'kaiming'
   Decider = require('../weight-init')(Decider, method)

   attention = nn.RecurrentAttentionDynamic(rnn, locator, Decider, opt.rho, {opt.hiddenSize}, opt.discount, opt.e, opt.greedy)
   ...
   print('classifier...')
   -- classifier
   agent2 = nn.Sequential()
   agent2:add(nn.SelectTable(1))
   agent2:add(nn.SelectTable(-1))
   agent2:add(classify)
   agent2:add(nn.HardTanh(-10,10))
   agent2:add(nn.LogSoftMax())
   
   concat3 = nn.ConcatTable():add(agent2):add(nn.SelectTable(2))
   ...
   seq:add(nn.Add(1))
   concat = nn.ConcatTable():add(nn.SelectTable(1)):add(seq):add(nn.SelectTable(2))
   concat2 = nn.ConcatTable():add(nn.SelectTable(1)):add(concat)

   Agent:add(concat2)
   
   VRClassReward = nn.VRClassRewardlzc
else
   ...
   VRClassReward = nn.VRClassReward
end
...
train = dp.Optimizer{
   loss = nn.ParallelCriterion(true)
      :add(nn.ModuleCriterion(nn.ClassNLLCriterion():cuda(), nil, nn.Convert())) -- BACKPROP
      :add(nn.ModuleCriterion(VRClassReward(Agent, opt.rewardScale, nil, opt.gama):cuda(), nil, nn.Convert())) -- REINFORCE
   ,
   ...
   callback = function(model, report)
      ...
      -- model:rmsprop() -- affects gradParams
...
if opt.testOnly then 
    xp:setup()
    numOfStep = Agent:get(1):get(1):get(2).numOfStep
    for i = 1, opt.rho do
        numOfStep[i] = 0
    end
    xp._tester:propagateEpoch(ds:testSet(), xp:report())
    report = xp:report()
    print('accuracy:' .. report.tester.feedback.confusion.accuracy)
    print(numOfStep)
else
    xp:run(ds)
end
