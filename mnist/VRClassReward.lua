...
------------------------------------------------------------------------
local VRClassReward, parent = torch.class("nn.VRClassRewardlzc", "nn.Criterion")

function VRClassReward:__init(module, scale, criterion, gama)
   parent.__init(self)
...
function VRClassReward:updateOutput(inputTable, target)
   ...
   local step = self:toBatch(inputTable[3], 1)
   ...
   step1 = torch.pow(self.gama, step)
   self.reward:cmul(step1)
